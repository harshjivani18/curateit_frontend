...
const SingleListItem = ({item,handleOpenGem, propertyOrder, collectionName, setOpenDrawer, setGemSingleIdSingleId, page, checkedBookmark, showComment, showAddToBookmark, user,isSharedAndAllowEdit,permissions,isFilter,imgSrc,favSrc, mediaImgsrc,s3Src,handleNavigate,handleNavigateMediaType,isMobileView,handleCheck,openGemInWindow,gemOnClickEvent}) => {
    ...
    if(isDragging){
        return(
            <div ref={setNodeRef} style={style} className="h-[130px] opacity-40 bg-[#dedede] rounded-md border border-solid border-[#DADEE8]">
    ...
    if((page === 'collection' && isSharedAndAllowEdit && !isFilter) || (page === 'tags' && isSharedAndAllowEdit && !isFilter)){
        return (
          ...
          >
            <div
              className={`bg-white cursor-pointer rounded-[8px] flex items-center mb-[16px] group relative shadow-lg transition-all hover:-translate-y-0.5 hover:shadow-2xl touch-none`}
              onClick={(e) => {
            ...
            >
              ...
              <div
                ...
                className={`p-1 absolute top-0 left-0 flex justify-between items-center h-fit ${
