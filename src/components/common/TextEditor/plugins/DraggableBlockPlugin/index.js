...
function isOnMenu(element) {
  return !!element.closest(`.${DRAGGABLE_BLOCK_MENU_CLASSNAME}`) || !!element.closest('.draggable-other-opts');
}
...
function useDraggableBlockMenu(
  ...
  const onDeleteElement = () => {
    ...
    // setShowDeleteOver(false);
  }

  // const onShowFlyoutMenu = () => {
  //   setShowDeleteOver(true);
  // }

  const onShowFloatMenu = () => {
    editor.update(() => {
      const node = $getNearestNodeFromDOMNode(draggableBlockElem);
      if (node) {
        const paraNode = $createParagraphNode();
        paraNode.append($createTextNode("/"));
    ...
    <>
      {/* <div className='flex flex-col'> */}
        <div className='flex flex-row w-[fit-content]'>  
          <div
            className="icon draggable-block-menu flex flex-row items-center justify-center"
            ref={menuRef}
            draggable={true}
            onDragStart={onDragStart}
            onDragEnd={onDragEnd}>
              {isEditable && <div className='flex flex-row items-center justify-center cursor-pointer'>
                <TrashIcon className="h-[16px] w-[16px] mr-1 cursor-pointer" onClick={onDeleteElement} />
                <PlusIcon className="h-[16px] w-[16px] cursor-pointer" onClick={onShowFloatMenu} />
              </div>}
              <div className={isEditable ? 'icon-block' : ''} />
          </div>
