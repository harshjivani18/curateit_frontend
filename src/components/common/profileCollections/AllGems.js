...
import { 
  getBookmarkInCollections, 
  getCollectionFilteredBookmark, 
  getParentPublicCollectionInProfile, 
  setProfileSubCollection, 
  setSubCollectionsPageDetails, 
  updateCollectionPageConfig 
...
const AllGems = ({ user }) => {
  ...
  const { profileSubCollection } = useSelector((state) => state.collections);
  ...
  const [selectedSubCollection, setSelectedSubCollection] = useState((profileSubCollection && profileSubCollection?.id) ? profileSubCollection : null);
  ...
  useEffect(() => {
    ...
    if(selectedSubCollection && selectedSubCollection?.id){
      const getCall = async () => {
            ...
            setLoading(true);
            setCheckedBookmark([])
            const res1 = await dispatch(getBookmarkInCollections(selectedSubCollection?.id, page))
            if(res1){
              setAllBookmarks([...allBookmarks, ...res1?.payload?.data?.collection?.gems]);
              setLoading(false);
              if (res1?.payload?.data?.totalBookmark <= allBookmarks.length + res1?.payload?.data?.collection?.gems?.length) {
                setHasMore(false);
              }
            }
            const configRes = res1?.payload?.data?.collection
            const data = getPropertiesData(res1?.payload?.data?.collection, "collection");
            setTotalCount(res1?.payload?.data?.totalBookmark);
            setFilterArr(configRes?.configColl?.filter);
            setSortArr(configRes?.configColl?.sort);
            setPropertyShown(data?.PS);
            setPropertyOrder(data?.PO);
            setPropertyHidden(data?.PH);
            setLayout(configRes?.configColl?.layout);
            ...
            setGroupLayout(configRes?.configColl?.groupLayout);
            setSubGroupLayout(configRes?.configColl?.subGroupLayout);
            setColumnColor(configRes?.configColl?.columnColor);
            setLoadingState(false);
        }
        if(hasMore && !loading){
          getCall()
  ...
  useEffect(() => {
    if(!selectedSubCollection){
      const getCollections = async () => {
        ...
        const res = await dispatch(getParentPublicCollectionInProfile(subCollectionPage))
  ...
  useEffect(() => {
    ...
    const getCall = async () => {
      ...
      if(selectedSubCollection && selectedSubCollection?.id){
        const res = await dispatch(getCollectionFilteredBookmark(aggregatedData.filterBy,aggregatedData.queryBy,aggregatedData.termType,filterPage,selectedSubCollection?.name,sortby,orderby))
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dispatch, filterPage,filterHasMore]);

  const updatePageConfig = async (obj) => {
    setPageConfig(obj);
    if(selectedSubCollection && selectedSubCollection?.id){
      dispatch(updateCollectionPageConfig(obj))
    } else{
      dispatch(savePageConfig(obj));
    }
  };
  ...
  const handleSelectSubCollection = () => {
    dispatch(setProfileSubCollection(null))
  ...
  const handleFilterSave = async () => {
    ...
    if(selectedSubCollection && selectedSubCollection?.id){
      const res = await dispatch(getCollectionFilteredBookmark(aggregatedData.filterBy,aggregatedData.queryBy,aggregatedData.termType,1,selectedSubCollection?.name,sortby,orderby))
      if(res.error === undefined){
  ...
  const handleFilterRemove = async (i) => {
    ...
    if (arr.length === 1 && sortArr.length === 1) {
      ...
      if(selectedSubCollection && selectedSubCollection?.id){
      const res = await dispatch(getCollectionFilteredBookmark('','','',1,selectedSubCollection?.name,sortArr[0].sortby,sortArr[0].orderby))
      if(res.error === undefined){
        setFilteredBookmarks(res?.payload?.data?.finalRes)
        setFilterTotalCount(res?.payload?.data?.totalCount)
      }
      ...
      profilePageFilteredBookmarks(
        ...
        '',
        1,
        sortArr[0].sortby,sortArr[0].orderby,
      )
      );
      if (res.error === undefined) {
      setFilteredBookmarks(res?.payload?.data?.finalRes);
  ...
  const handleSortSave = async () => {
    ...
    setFilterHasMore(true)
    if(selectedSubCollection && selectedSubCollection?.id){
      const res = await dispatch(getCollectionFilteredBookmark(filterBy,queryBy,termType,1,selectedSubCollection?.name,sortArr[0].sortby,sortArr[0].orderby))
        if(res.error === undefined){
          setFilteredBookmarks(res?.payload?.data?.finalRes)
          setFilterTotalCount(res?.payload?.data?.totalCount)
        }
  ...
  const handleSortRemove = async (i) => {
    ...
    setFilterHasMore(true)
    if(selectedSubCollection && selectedSubCollection?.id){
      const res = await dispatch(getCollectionFilteredBookmark(aggregatedData.filterBy,aggregatedData.queryBy,aggregatedData.termType,1,selectedSubCollection?.name,'',''))
      if(res.error === undefined){
        setFilteredBookmarks(res?.payload?.data?.finalRes)
        setFilterTotalCount(res?.payload?.data?.totalCount)
      }
      ...
      const res = await dispatch(
      profilePageFilteredBookmarks(
        aggregatedData.filterBy,aggregatedData.queryBy,aggregatedData.termType,
        1,
        '','',
      )
      );
  ...
  const handleGemOnClickEvent = async (item) => {
    ...
    const data = {
      preferences: { ...userData?.preferences, gemOnClickEvent: item },
    };
    ...
    <div>
      {loadingState ? (
        ...
        <>
          ...
          <SubCollectionComponent fromPage="profile" />
