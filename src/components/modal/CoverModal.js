...
import { GALLEY_UPLOAD_COLORS,debounceFunction } from "@utils/constants";
...
const CoverModal = ({
  ...
  const UNSPLASH_API = process.env.NEXT_PUBLIC_UNSPLASH_API;
  useEffect(() => {
    const getCall = async () => {
      setLoading(true);
      const res = await axios.get(
        `${UNSPLASH_API}/photos/random?count=12&&client_id=${UNSPLASH_API_ACCESS_KEY}`
      );
  ...
  const getSearchPhotos = useCallback(
    async (value) => {
      ...
      setLoading(true);
      const res = await axios.get(
        `${UNSPLASH_API}/search/photos?query=${value}&per_page=12&client_id=${UNSPLASH_API_ACCESS_KEY}&page=${page}`
      );
      ...
      <div className="w-full flex flex-wrap py-0">
        {GALLEY_UPLOAD_COLORS.map((item) => (
          <div
            ...
            key={item.id}
            onClick={(e) => handleGalleryUploadChange(e)}
          >
            <div
        ...
        >
          ...
          >
            <div>
              ...
              <Button variant="mt-2 primary" disabled={loadingCoverImg}>
                Browse File
              </Button>
            </div>
          </div>
    ...
    <>
      {openCoverModal && (
        ...
        >
          <>
            {collectionCoverImage &&
              (collectionCoverImage?.type === "upload" ||
                collectionCoverImage?.type === "unsplash") && (
                <div className="flex justify-between w-full">
                  <div>
                    <img
                      src={collectionCoverImage?.icon?.replace(
                        NEXT_PUBLIC_STATIC_S3_BASE_URL,
                        `${NEXT_PUBLIC_STATIC_S3_BASE_URL}/800x800_cover`
                      )}
                      alt="cover"
                      className="h-[100px]"
                    />
                  ...
                  <div className="text-right">
                    <Button
                      ...
                      onClick={handleDeleteCoverS3Link}
                      disabled={loadingCoverImg}
                    >
                      Remove Cover
                    </Button>
            ...
            {collectionCoverImage &&
              collectionCoverImage?.type === "gallery" &&
              collectionCoverImage?.icon !== "#fff" && (
                <div className="flex justify-between w-full">
                  <div
                    className="w-[100px] h-[100px]"
                    style={{ background: collectionCoverImage?.icon }}
                  ></div>
