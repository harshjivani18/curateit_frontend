...
const EmbedTag = ({items,pageNumber}) => {
    ...
    useEffect(() => {
        if(items){
            setData(items?.gems || [])
            setCollectionName(items?.tag)
            setHasMore(items?.gems.length > 0)
        }
    },[items])
    ...
    const loadMoreData = async () => {
        ...
        try {
            ...
            if(res.error === undefined){
                setLoading(false)
                setData(prevData => [...prevData, ...res?.data?.tags?.gems || []])
                setCollectionName(res?.data?.tags?.tag)
                setHasMore(res?.data?.tags?.gems.length > 0)
            }else{
                setLoading(false)
                setHasMore(false)
            }
        ...
        <div className="border border-2 w-full">
            ...
            >
                ...
                <div>
                    ...
                    onClick={(e) => {
                        e.stopPropagation();
                        window.open(`${process.env.NEXT_PUBLIC_BASE_URL}/u/${username}/embed-tag-more/${id}?view=${view}`, "_blank");
                    }}
