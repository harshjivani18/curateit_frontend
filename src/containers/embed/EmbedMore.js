...
const EmbedMore = () => {
    ...
    useEffect(() => {
        const getCall = async () => {
            ...
            try {
                const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/embed-collection/${id}?isEmbed=true&page=${1}&perPage=20`)
                if (res.error === undefined) {
                    setLoadingState(false)
                    setData(res?.data?.collection?.gems || [])
