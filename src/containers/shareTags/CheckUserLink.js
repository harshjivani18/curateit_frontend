...
import { checkTagViaLink, clearAllTags, getSharedTags, resetSharedTags } from "@actions/tags";
...
const CheckUserLink = () => {
    ...
    useEffect(() => {
        if(session && session?.userId && inviteId){
            const getCall = async () => {
                ...
                const res1 = await dispatch(checkTagViaLink(inviteId,tagId))
                ...
                if(res1.error === undefined && res1.payload.data.status === 200){
                    ...
                    setError(res1?.payload?.data?.msg)
                    setLoadingSessionCheck(false);
                }
            }
    ...
    const submitDataRegister = async (e) => {
        ...
        if(res.error === undefined){
            ...
            if(res1.error === undefined && res1?.payload?.data?.status === 200){
                ...
                message.error(`You are not authorized to access that linked tag but your account has been created successfully.`)
                setLoading(false)
            }
        }else{
            setLoading(false)
    ...
    const submitData = async (e) => {
        ...
        if(res.error === undefined){
            ...
            if(res1.error === undefined && res1.payload.data.status === 200){
                ...
                message.error(res1.payload.data.msg)
                session.clear();
    ...
    if(!loadingSessionCheck && !error){
        ...
        <div>
            <SocialLoginOptions page={'tag-link'} inviteId={inviteId} tagId={tagId}/>
        </div>
