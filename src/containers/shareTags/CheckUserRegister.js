...
import { checkTagExpiration, clearAllTags, getSharedTags, resetSharedTags } from "@actions/tags"
...
const CheckUserRegister = () => {
    ...
    if (session && session?.emailAddress === emailId) {
      const getCall = async () => {
        ...
        const res1 = await dispatch(
          checkTagExpiration(emailId, tagId)
        );

        if (res1.error === undefined && res1.payload.data.status === 200) {
          await dispatch(getSharedTags());
          const tObj = res1?.payload?.data?.data?.[0]
          const slug = tObj?.slug || slugify(tObj?.tag || "", { lower: true, remove: /[&,+()$~%.'":*?<>{}]/g })
          navigate.push(`/u/${session.username}/tags/${tObj?.id}/${slug}`)
          setLoadingSessionCheck(false);
        } else {
          setLoadingSessionCheck(false);
          setError(res1.payload.data.msg ||"Tag shared is expired. Please contact the owner")
        }
      };
      getCall();
      return;
    ...
    const submitData = async (e) => {
        ...
        if(res.error === undefined){
            const res1 = await dispatch(checkTagExpiration(email,tagId))
            ...
            if(res1.error === undefined && res1.payload.data.status === 200){
               ...
               message.error(res1.payload.data.msg || 'Tag shared is expired. Please contact the owner')
               session.clear();
    ...
    return(
        ...
        <main className='px-16 mt-[14px]'>
            ...
            <nav className="-mb-px flex space-x-1 justify-around" aria-label="Tabs">
                <span className="border-blue-500 text-blue-600 whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm w-[48%] text-center">Login to access</span>
            </nav>
            </div>

            <div>
                {/* <SocialLogin  /> */}
                <SocialLoginOptions page={`tag-mail`} email={emailId} tagId={tagId}/>
            </div>

            <h6 className='text-sm text-center text-gray-500'>or sign in with email </h6>
            <>
        {/* <Error /> */}
        <div className='py-4'>
            <div className='mb-4'>
